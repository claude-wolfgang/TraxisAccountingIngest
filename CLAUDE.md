...
- **[NEEDS WOLFGANG] srv-01 access recovery — Mon + Tue work** (added 2026-05-17). srv-01 has no video output on any of three monitors with multiple DP-to-HDMI cables across both DP ports — Dell POST logo never appears. The box is otherwise fully alive: TCP/22 answers, sshd is listening, Windows is booted. But Claude's pubkey was rejected on the SSH attempt — `administrators_authorized_keys` got invalidated somehow. CMOS clear didn't help. Hardware ordered to unblock: UGREEN M.2 NVMe USB enclosure (arrives Mon 10am–3pm) + SANSUI 27" monitor with native DP input + wall mount + wired keyboard (Tue). **Monday plan:** Wolfgang pops srv-01's M.2 (currently in his hand) into the UGREEN enclosure → USB into .178 → Claude edits `<letter>:\ProgramData\ssh\administrators_authorized_keys` to re-add pubkey + `icacls`-fix ACLs → reinstall M.2 in srv-01 → Claude SSH to `10.1.1.161` to confirm. **Tuesday plan:** wall-mount SANSUI upstairs, DP-to-DP cable from srv-01's DP1 → monitor's DP input. If video appears, the original failure was the DP-to-HDMI cable path (case closed, srv-01 has a permanent console). If still no video, decision point: swap srv-01's chassis (second 7060 Micro can become srv-01) or accept permanent headless. **Still need to source:** DP-to-DP cable (~$10, most monitors don't include) + wired mouse if no spare. .71 stays primary in production until this resolves; no production impact.
- **[NEEDS WOLFGANG] srv-01 cutover (blocked on access recovery above)** — srv-01 is upstairs, headless, at static IP `10.1.1.161`, `TRAXIS-SRV-01`. TraxisOverseer NSSM service installed but **stopped, Start mode = SERVICE_DEMAND_START**. To cut over from .71: (1) stop .71's Overseer + service_wrapper. (2) Copy state DBs to `T:\traxis\data\` on srv-01: `C:\FASData\monitoring.db` + `shophub.db`, `22/tool-kiosk/data/tooling.db`, `25/audit.db`, `31/photo-uploader/data/` tree, `lathe_programs.json`, `document_mappings.json`. (3) Set `TRAXIS_FOCAS_DB` env var to the copied path. (4) Update kiosk PC bookmarks (10.1.1.142, etc.) from `10.1.1.71:port` → `10.1.1.161:port`. (5) On srv-01: `nssm set TraxisOverseer Start SERVICE_AUTO_START; nssm start TraxisOverseer`. (6) Soak 48-72hr. (7) Delete `TraxisOverseer.lnk` from .71's Startup folder to decommission .71's services role. **Note:** srv-01 will also need `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` env vars set for Telegram alerting to work there.
