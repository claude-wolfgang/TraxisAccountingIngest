...
- **[NEEDS WOLFGANG] Tool kiosk on-site work — bundle with srv-01 migration weekend** (surfaced 2026-05-19). All code/docs deployed via Dropbox; remaining work is keyboard-on-.141. On the weekend with srv-01: (1) On .141, set `TOOLKIOSK_BACKEND_URL=http://10.1.1.161:5001` in `.traxis.env` (new env var the launcher reads). (2) Calibrate touch input: Control Panel → Tablet PC Settings → Setup → tap touchscreen. (3) Double-click `22. Tool Assembly Management/tool-kiosk/install_autostart.bat` to register the `TraxisToolKiosk` scheduled task at logon (idempotent). (4) `schtasks /Run /TN TraxisToolKiosk` to start without reboot. (5) From any host, `curl http://10.1.1.161:5001/api/health` → confirm `kiosk_heartbeat_age_seconds` is ≤60. (6) Bounce Flask once via Overseer to clear lingering test-heartbeat state from 2026-05-19 (`curl -X POST http://<host>:8060/api/services/ToolAssemblyKiosk/restart`). (7) Restart Overseer to activate the new validator that flags "Touchscreen silent for Xs" when the kiosk goes dark. Launcher now auto-detects which monitor is the touchscreen via the Win32 Pointer Device API — no need to keep the touchscreen as primary.
- **srv-01 service migration — Monday** (updated 2026-05-18). SSH access recovered (`ssh traxi@10.1.1.161` via .71's RSA pubkey). srv-01 readiness confirmed: NSSM + TraxisOverseer registered (stopped/manual), Python 3.14 + all deps, `T:\traxis\services\` seeded, Telegram env vars set, `.traxis.env` in place. **Remaining steps:** (1) Stop .71's Overseer. (2) Copy state DBs to `T:\traxis\data\` on srv-01 via SCP: `monitoring.db`, `shophub.db`, `tooling.db`, `audit.db`, `31/photo-uploader/data/` tree, `lathe_programs.json`, `document_mappings.json`. (3) Set `TRAXIS_FOCAS_DB` env var to `T:\traxis\data\monitoring.db`. (4) `nssm set TraxisOverseer Start SERVICE_AUTO_START; nssm start TraxisOverseer`. (5) Update kiosk PC (10.1.1.141) `TOOLKIOSK_BACKEND_URL` env var from `http://10.1.1.71:5001` → `http://10.1.1.161:5001` (and any other kiosk bookmarks). (6) Soak 48-72hr. (7) Decommission .71's services role. FocasMonitor stays on .71 for now (deferred).
- **[NEEDS WOLFGANG] srv-01 video output — Tue work** (updated 2026-05-18). Video still dead (no POST on any monitor). SANSUI 27" with native DP input arrives Tuesday. Plan: wall-mount SANSUI upstairs, DP-to-DP cable from srv-01's DP1 → monitor's DP input. If video appears, case closed (DP-to-HDMI cables were the problem). If still no video, decision point: swap chassis or accept permanent headless. **Still need to source:** DP-to-DP cable + wired mouse.
...
- **Investigate AgentScheduler audit failure** (surfaced 2026-05-17). AgentScheduler is cycling degraded every ~7 min: its audit subprocess exits with code 1. Overseer auto-restarts it, but it never recovers. The audit failure is masked — no stderr captured. Worth investigating the root cause in `run_audit.py` or its dependencies.
- **Investigate TimeTrackerDashboard weekend flapping** (surfaced 2026-05-17). On weekends/off-hours, TimeTracker goes "Data stale" → degraded → Overseer restarts → healthy for a few minutes → stale again. The restart is pointless — there's no fresh data to fetch when nobody is clocked in. Consider either (a) relaxing the staleness threshold outside business hours, or (b) skipping auto-restart for expected off-hours staleness.
