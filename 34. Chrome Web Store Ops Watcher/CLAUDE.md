...
Status: **Phase 1 implemented (manual run). Phase 2 (scheduling) pending.**
...
| File | Purpose |
|------|---------|
| `cws_watcher.py` | Main poller. Thin Graph client + classifier + SQLite logger + flag writer. ~210 lines, single file. |
...
Produces: `cws_events.db` (SQLite event log), flag files in `flags/` for Overseer pickup
...
- **Scheduling.** Windows Task Scheduler firing `pythonw.exe cws_watcher.py` every 4h is the simplest path. Per P32 lesson, register the task with direct `pythonw.exe` invocation (not a `.bat` wrapper) to avoid console-window flash. Promote to Overseer-managed service later if 4h cadence proves too coarse.
- **Overseer integration.** Add a heartbeat file or `/health` endpoint so P1 dashboard surfaces flag-file count and last-poll timestamp.
- **Optional Telegram routing.** P25 AgentScheduler can forward critical events to your phone for off-hours suspension alerts.
- **Approved-event auto-clear.** When a `submission_approved` event arrives matching an earlier `submission_rejected` for the same item, auto-resolve the older flag.
