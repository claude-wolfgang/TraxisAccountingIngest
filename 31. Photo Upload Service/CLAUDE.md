...
- **Fix the dev-server reboot cycle (waitress + /api/shutdown)** — every code change to `app.py` that requires a restart risks wedging port 5003 (Werkzeug `app.run()` can't shut down cleanly; `Popen.terminate()` hard-kills mid-flight connections; dead PID retains the LISTEN socket; new spawn collides). 2026-05-04 evening proved this concretely. Today's Overseer-driven restart happened to be clean (single fresh listener, no zombies), but the underlying fragility is unchanged. Two-part fix: **(a)** swap `app.run()` for `from waitress import serve; serve(app, host=..., port=...)` — production-grade WSGI server with proper socket lifecycle and per-connection timeouts; **(b)** add a `POST /api/shutdown` endpoint that closes the listen socket and `sys.exit(0)`s, then teach `Overseer._stop_process` to call it FIRST with a 5s budget before falling back to terminate/kill. Both pieces small. Overseer's adoption-of-zombie-PID bug was fixed 2026-05-04 (skips dead PIDs in `_find_pid_by_port`); this remaining work is on the photo-uploader side. (Cross-cutting: applies to every Werkzeug-backed Flask service Overseer manages — see root CLAUDE.md.)
