...
- **COTS-page upload selectors unknown — DOM inspection needed** — `_fetch_cots` URL fix landed 2026-05-14 so COTS searches now return the correct `/procnc/ots/{type}/{number}` URL (e.g. `/ots/PAC/223`). But the worker still can't drive the COTS detail page: `_try_upload_at` never finds a CHECKOUT/SAVE button within 25s. WO/part/equipment/tool flows were DOM-inspected back when the worker was written (see `inspect_upload.py`); COTS was never exercised end-to-end. Adapt `inspect_upload.py` to navigate visible-mode Chrome to a COTS detail page (use photo #18 / PAC-223 as the live failing case), dump the page DOM + screenshot, identify whether COTS pages use a different button label, a tabbed layout requiring a click first, or a different editor entirely. Then either patch `_try_upload_at` with COTS-specific button matching or branch off to a COTS-specific helper. Photo #18's row is already reset to `pending` with the corrected URL so a fix can be verified by watching the queue.
...
Contracts: Port 5003 must not conflict with other services. Photo storage at photo-uploader/data/photos/ syncs via Dropbox. Upload worker consumes ProShop web UI via headless Selenium. Written description URL pattern: `{BASE}/procnc/parts/{customer}/{partNumber}$formName=writtenDescription&opId={opNumber}`. **WO-page fallback (added 2026-05-13):** when the op's writtenDescription URL produces no usable CKEditor form (no CHECKOUT button within 25s, OR checkout fails, OR CKEditor not ready within 30s), worker retries on `{BASE}/procnc/workorders/{wo}` and reports success with a non-error info note ("Operation had no written description — uploaded to WO main page instead.") rendered neutral grey on /queue. Only triggers for entity_type=workorder. **ProShop's per-CKEditor-field ceiling is ~240KB**: base64-inline shrink loop tries 6 dimensions × 4 qualities (1200→360 × 75→30); after 2 photos stack onto the same WO main page via fallback, subsequent attempts genuinely cannot fit and fail with "page may be full". OAuth scope requires: parts:rwdp, workorders:rwdp, users:r, tools:rwdp, toolpots:r, fixtures:r, ots:r, equipment:r, nonconformancereports:r. Print label endpoint POSTs `{image_base64, copies, label_name}` to print service (same payload as P30). Label designs match P30 exactly: material/equipment/tool/box auto-width 128px tall, COTS fixed 450×128px, all at 180 DPI for 24mm tape. P30 extension's "Buy" button POSTs `{entity_type, entity_id, qty, unit_cost?, vendor?, brand?}` to /api/queue-order; backend either auto-approves under-threshold orders or drafts a vendor quote-request email (capped 3/day per vendor) when unit_cost is missing. **Tool entity requests:** if no vendor scraped, server defaults to AJ Rodco; quote-request email subject/body uses `{approvedBrand} {vendorToolId/EDP}` from ProShop tool library (top approvedBrands record), with the description below — falls back to description, then entity_id, if approvedBrands is empty. Outlook draft folder lookup is root-level only — folder must stay at the mailbox root. **`error_message` column dual-use:** `database.update_photo_status(id, "uploaded", note)` preserves the message when status is `uploaded` so the queue UI can render it neutral grey via `.info-text`; on `failed` it's rendered red via `.error-text`. Don't write empty strings to `error_message` for successful uploads — pass `None` (default) instead.
