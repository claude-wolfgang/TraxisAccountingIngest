...
- **[NEEDS WOLFGANG] Fix the dev-server reboot cycle** — every code change to `app.py` that requires a restart risks wedging port 5003 (Werkzeug `app.run()` can't shut down cleanly; `Popen.terminate()` hard-kills mid-flight connections; dead PID retains the LISTEN socket; new spawn collides). Two-part fix: **(a)** swap `app.run()` for `from waitress import serve; serve(app, host=..., port=...)` — production-grade WSGI server with proper socket lifecycle; **(b)** add a `POST /api/shutdown` endpoint that closes the listen socket and `sys.exit(0)`s, then teach `Overseer._stop_process` to call it FIRST with a 5s budget before falling back to terminate/kill. Both pieces small. Until done, avoid live-restart deploys on this service. May warrant rolling out to the other Flask services Overseer manages.
