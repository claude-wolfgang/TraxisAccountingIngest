#!/usr/bin/env python3
"""
ProShop Time Tracking & Profitability API Test
...
Usage:
    python proshop_time_tracking_test_v1_0_1.py
"""
...
def test_work_order_time_tracking(token: str, wo_number: str) -> dict:
    ...
    if "errors" in result:
        ...
        for op in ops.get("records", [])[:5]:
            ...
            desc = op.get('operationDescription') or ''
            print(f"     [{complete}] Op {op.get('operationNumber')}: {desc[:40]}")
    else:
...
def test_work_cells(token: str) -> dict:
    ...
    if "errors" in result:
        ...
        for wc in wcs.get("records", [])[:15]:
            scheduled = "✓" if wc.get("isScheduledResource") else "✗"
            name = wc.get('shortName') or wc.get('commonName') or wc.get('description') or ''
            dept = wc.get('department') or ''
            lathe = " (lathe)" if wc.get('isLathe') else ""
            print(f"     [{scheduled}] {wc.get('potId')}: {name[:35]}{lathe} | {dept}")
    else:
...
def main():
    print("=" * 60)
    print(f"ProShop Time Tracking & Profitability API Test v{VERSION}")
