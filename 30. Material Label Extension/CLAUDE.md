...
- **Tool pages** (`/procnc/tools/*`) — "Print Tool Label" button (orange)
...
tool-content.js       →  tool ID from URL, iframe scrape + DOM scrape + API fallback
...
**Note:** ProShop tool pages render form inputs inside iframes (unlike other page types). The tool content script tries three data sources in order: iframe `contentDocument` scraping, top-level DOM scraping, then GraphQL API fallback (using session cookie, same as material/equipment labels).
...
| `traxis-material-label/src/tool-content.js` | Tool button injection, iframe + DOM + API scraping |
...
| `BUY_BUTTON_REFERENCE.md` | Function map + 4 test recipes for the Buy button (spans P30 + P31). Read first when Buy-button behavior is discussed. |
...
- **[NEEDS WOLFGANG] CWS v1.6.1 in review — submitted 2026-05-22** — ZIP at `deployment/traxis-label-printer-v1.6.1.zip`. Patches host_permissions and `service-worker.js` `PHOTO_SERVICE` constant from the now-dead `.71:5003` to live srv-01 `.161:5003`; bumped from approved v1.6.0. Privacy policy at `claude-wolfgang/traxis-privacy` updated to match before submission (commit `b8eaa48`, live 2026-05-22). Likely flagged "in-depth review" again due to host_permissions change. If bounced, paste rejection text to redraft. Unlisted reviews typically clear 1-3 business days.
- **[NEEDS WOLFGANG] Dogfood v1.6.1 unpacked on .178, then walk the shop after CWS approval** — Wolfgang's .178 currently runs v1.6.1 as a developer unpacked load (CWS v1.6.0 toggled OFF). When CWS approves v1.6.1: (1) **Remove the unpacked card from `chrome://extensions` BEFORE re-enabling the CWS card** — otherwise both load and double-inject Buy/Print buttons on every page. (2) Run `deployment/deploy_client_cws.bat` (already baked with prod CWS ID `kepfeakajdfklilmpaidipmloclichjp`) right-click → Run as administrator on each shop PC, restart Chrome, verify v1.6.1 on `chrome://extensions` with the "Installed by enterprise policy" badge. End-to-end recipe lives in `BUY_BUTTON_REFERENCE.md` §7d.
- **Tighten Cost-column scrape in `buy-content.js`** — heuristic worked for LUB-116 but missed for THI-17 AND FIL-157 (second confirmed miss, 2026-05-14). Without unit_cost the order falls through to the manual-review or auto-quote-request path even when ProShop has a price visible. Inspect FIL-157 page DOM (Amazon vendor row) to learn the column layout and tighten the column-finder.
- **[Optional] Make `buy-content.js` iframe-aware on tool pages** (surfaced 2026-05-20). `scrapeBrand`/`scrapeVendor`/`scrapeUnitCost` only query top-level `document.querySelectorAll('th, td')`, but ProShop tool detail pages render the Purchasing table inside an iframe. Net effect on tools: scrapes are unreliable and sometimes leak the Vendor cell into the brand slot — produced "Aj Rod 43584TF" quote-request subjects on B23 today. P31's server-side fix now overwrites tool brand/edp from the ProShop GraphQL API regardless of what was scraped, so this is no longer a correctness issue, but the scraper still wastes effort and sends garbage payload fields. Mirror tool-content.js's iframe-contentDocument pattern (same-origin, so it works) if revisiting. Low priority while the server-side authority is in place.

## Interfaces

Produces: Material label PNGs, Box label PNGs, COTS label PNGs (450px wide), Equipment label PNGs, User label PNGs, Tool label PNGs — all 128px tall, auto-width (except COTS), as base64 PNG via Canvas API; purchase-order POST `{entity_type, entity_id, qty, unit_cost?, brand?, vendor?}` to P31's `/api/queue-order` (Buy button on COTS/Tools/Parts pages).
Consumes: ProShop WO page DOM, ProShop COTS page DOM, ProShop Equipment page DOM, ProShop User page DOM, ProShop Tool page DOM (via iframe contentDocument), ProShop Parts page DOM (Buy button), ProShop GraphQL API (session cookie), Brother PT-P700 print service at http://10.1.1.242:5002, P31 purchasing-queue service at http://10.1.1.161:5003 (Buy button — added in v1.6.0; host moved from .71 to srv-01/.161 in v1.6.1 on 2026-05-22).
Contracts: Print payload `{image_base64, copies, label_name}` to `/api/print-image` (same as P9/P22). Buy payload `{entity_type, entity_id, qty, unit_cost?, brand?, vendor?}` to `/api/queue-order` (P31). WO and Box labels use QR scheme `proshop://wo/{woNumber}` (same as P9). COTS, equipment, user, and tool labels encode full ProShop URL. All labels 128px tall at 180 DPI (PT-P700 24mm tape). Tool label uses three-layer data cascade: iframe contentDocument → top-level DOM → GraphQL API (`tools` query with session cookie). CWS extension ID used in registry policy on all shop PCs — changing it requires re-running deploy script. Privacy policy at https://claude-wolfgang.github.io/traxis-privacy/ — must be kept aligned with the manifest's `host_permissions` and `permissions` lists (mismatch is the #1 CWS rejection trigger; the policy lives in repo `claude-wolfgang/traxis-privacy` on GitHub Pages, separate from this code repo).
