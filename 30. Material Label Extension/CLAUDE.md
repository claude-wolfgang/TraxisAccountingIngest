...
- **[NEEDS WOLFGANG] CWS v1.6.0 — submitted for review 2026-05-14** — ZIP at `deployment/traxis-label-printer-v1.6.0.zip`, manifest description trimmed to 109 chars (132 cap), privacy policy at `claude-wolfgang/traxis-privacy` updated to disclose `10.1.1.71:5003`, host-permission justification and Data-usage form filled (Website content = Yes with LAN-only justification, all other categories = No). Google flagged "in-depth review may delay publishing" due to host permissions. If bounced, paste rejection text to redraft. Unlisted reviews typically clear 1-3 business days; shop PCs auto-update via ExtensionInstallForcelist after approval.
...
Consumes: ProShop WO page DOM, ProShop COTS page DOM, ProShop Equipment page DOM, ProShop User page DOM, ProShop Tool page DOM (via iframe contentDocument), ProShop Parts page DOM (Buy button), ProShop GraphQL API (session cookie), Brother PT-P700 print service at http://10.1.1.242:5002, P31 purchasing-queue service at http://10.1.1.71:5003 (Buy button — added in v1.6.0).
